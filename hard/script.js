...
/**
 *
 * #######################CHANGES WERE MADE HERE FROM VERSION 0
 */
...
Here is a change in aiSpot() routine from version 0 of the game. Now this function delegates the returning of index to aiThinking() subroutine which teaches the ai how to make the best move and not let the player win.

*/

function aiSpot() {
  //return emptySpots()[0];
  return aiThinking(board, ai).index;
}

/*

The aiThinking function works using the minimax algorithm, as explained at https://www.freecodecamp.org/news/how-to-make-your-tic-tac-toe-game-unbeatable-by-using-the-minimax-algorithm-9d690bad4b37/

*/

function aiThinking(thisBoard, player) {
  var availSpots = emptySpots(thisBoard);
  if (checkWin(thisBoard, player)) {
    return { score: -10 };
  } else if (checkWin(thisBoard, ai)) {
    return { score: 20 };
  ...
  for (var i = 0; i < availSpots.length; i++) {
    var move = {};
    move.index = thisBoard[availSpots[i]]; /*initialization? */
    thisBoard[availSpots[i]] =
      player; /* This helps to update the inboard in the next call below */
    if (player == ai) {
      var result = aiThinking(thisBoard, hu);
      move.score = result.score;
    } else {
      var result = aiThinking(thisBoard, ai);
      move.score = result.score;
    }
    thisBoard[availSpots[i]] = move.index;
    moves.push(move);
  }
  var bestMove;
  if (player === ai) {
...
/**
 *
 * ##################cHANGES WERE MADE UPTO HERE , FROM VERSION 0
 */
