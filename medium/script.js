...
function registerTurn(squareId, player) {
  ...
  if (player === hu) {
    document.getElementById(squareId.toString()).style.backgroundImage =
      "url('./zero.png')";
  } else {
    document.getElementById(squareId.toString()).style.backgroundImage =
      "url('./cross.png')";
...
/*
    ...
    For medium level, the empty spot is randomly choosen by the AI. In this is case its a game of chance.
...
function aiSpot() {
  ...
  let index = Math.floor(Math.random() * (y - 0) + 0);
  ...
  if (emptySpots().length == 0) {
    if (checkWin) {
      ...
      for (let i = 0; i < cells.length; i++) {
        cells[i].removeProperty("background-image");
      }
      return false;
    } else {
      for (var i = 0; i < cells.length; i++) {
